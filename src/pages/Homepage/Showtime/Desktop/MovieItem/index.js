    ...
    <div style={{
      ...
      <div className="film">
        ...
        <div className="film__content">
          <div className={`film__name ${movie.thoiLuong ? "" : "not_hide"}`}>
            <div className="name">
              <p><span className="c18">C18</span>{movie.tenPhim}</p>
            </div>
            <p className="pt-2">
              {movie.thoiLuong ? <span className="text_info">{movie.thoiLuong} phút - Tix {movie.danhGia}</span> : <span className="text_info">Tix {movie.danhGia}</span>}
            </p>
          </div>
          <div className={`film__button`}>
            {/* nếu thoiLuong = undefined => phim hiện không có lịch chiếu */}
            {(movie.thoiLuong || comingMovie) && <Link style={{ background: comingMovie ? "#60c5ef" : "#fb4226", }} to={{ pathname: `/phim/${movie.maPhim}`, state: { comingMovie } }}>{comingMovie ? "THÔNG TIN PHIM" : "MUA VÉ"}</Link>}
          </div>
        </div>
      </div>
    </div>
